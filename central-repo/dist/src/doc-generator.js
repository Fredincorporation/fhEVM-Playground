 ...
 */
export function extractJSDocBlocks(solidityCode) {
    const blocks = [];
    const docRegex = /\/\*\*\n([\s\S]*?)\n\s*\*\//g;
    let match;
 ...
 */
function parseDocBlock(docContent) {
 ...
 */
export function generateContractMarkdown(contractName, solidityCode, categoryDescription) {
 ...
 */
export function generateProjectReadme(projectName, category, contractNames) {
 ...
 */
export function generateCategorySummary(categoryData) {
    ...
    markdown += `## Keywords\n\n`;
    markdown += categoryData.keywords.map((k) => `- \`${k}\``).join('\n');
    markdown += '\n\n';
    return markdown;
}
